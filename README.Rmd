...
The length between the two points plotted on the image during the scaling process $L_{img}$ is calculated using Pythagorean Theorem $L_{img} = \sqrt{(x_{max} - x_{min})^2 + (y_{max} - y_{min})^2}$ and this length is converted to a scaling coefficient by dividing the image line length $L_{img}$ by the real-world length $L_{real}$
